...
// launch path downloads only the first missing disc and leaves the rest as 0-byte
// stubs behind a full .m3u — so "downloaded but incomplete" is a first-class,
// VALID on-card state. This file is the offline (filesystem + manifest only) view
// of that state: what the --check-rom gate, the --fetch-discs modes, and the
// daemons' --prefetch-discs enumeration all key off. No network, no device.
package catalog
...
// disc set is incomplete: the .m3u is real, but at least one referenced disc is
// absent or a 0-byte stub.
...
func IncompleteMultiDiscDownloads() []IncompleteM3U {
	...
	for rel, e := range man.Entries {
		...
		total, present := M3UCompleteness(abs)
