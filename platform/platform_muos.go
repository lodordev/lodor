...
// fix; muOS resolves discs via the .m3u's relative lines, written in lockstep, so the
// folder never needs to be browsable). Returns "" when the ROM has no platform slug.
